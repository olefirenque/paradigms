...
public class BinarySearchSpan {
    ...
    public static void main(final String[] args) {
        ...
        // Pred: Pred*
        // Post: l = min {i | array[i] <= x}
        var l = iterativeBinarySearchLeft(array, x);
        // Pred: Pred*
        // Post: r = min {i | array[i] >= x}
        var r = recursiveBinarySearchRight(array, x);
    ...
    // Inv: Pred
    // Post: R = min {i | array[i] <= x}
    public static int iterativeBinarySearchLeft(final int[] array, final int x) {
    ...
    // Inv: R ∈ [l;r) && sorted
    // Post: R = max {i | array[i] >= x}
    public static int recursiveBinarySearchRight(final int[] array, final int x) {
        Objects.requireNonNull(array);
        // Pred: array != null && ∀ i,j ∈ [0;array.length): (i < j -> array[i] >= array[j])
        // Post: R = max {i | array[i] <= x}
        return recursiveBinarySearchRight(array, x, -1, array.length);
    }
    ...
    // Inv: R ∈ [l;r) && sorted
    // Post: R = max {i | array[i] >= x}
    public static int recursiveBinarySearchRight(final int[] array, final int x, final int l, final int r) {
        Objects.requireNonNull(array);
        if (r - l == 1) {
            // Pred: r - l == 1
            // Post: Inv && r - l == 1 -> R = max {i | array[i] >= x}
            return l;
        }
        // Pred: r - l != 1
        // Post: m == (l + r) / 2 && Inv
        ...
        if (array[m] >= x) {
            ...
            // Post: R = max {i | array[i] >= x}
            return recursiveBinarySearchRight(array, x, m, r);
        }
        // Pred: (l' == l && r' == m) && m < r -> Inv && (r' - l' > r - l) && array[r] < x && Pred*
        // Post: R = max {i | array[i] >= x}
        return recursiveBinarySearchRight(array, x, l, m);
    }
}
